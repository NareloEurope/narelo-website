...
The site is published in **English (default) and Spanish**, in that order in
the selector.
...
- **German is switched off** (Vivien, 2026-09-08) and **Hungarian** before it
  ...
  bring one back, add `'de'` or `'hu'` to `LANGS` and `LANG_META` in
  `content/locales.ts` and re-import it in `content/dictionary.ts`. **Keep both
  updated when you add a key**, or turning one back on becomes a rewrite.
- The translation files mirror the English key for key. `content/dictionary.ts`
  ...
  translations, German and Hungarian included.
...
- **Jessica Fletcher is team, not a founder** (Vivien, 2026-09-05). The September audit listed her alongside Natalie and Vivien; that was wrong. She is Senior Educator and lives in `proof.team` in `content/membership.ts`, under her own "The team" label below the founders, so the page shows the distinction. Never present her as a founder. Mercedes Carrasco Galera (Brand Experience Manager) was also in `proof.team` until 2026-09-10, when she left Narelo and was removed from the site (Vivien). Do not re-add her.
