 ...
 */
export default function PriceReveal({ label, children }: { label: string; children: React.ReactNode }) {
  const [open, setOpen] = useState(false);

  return (
    <div className="mt-2">
      <button type="button" onClick={() => setOpen((v) => !v)} className="link-line eyebrow !tracking-normal text-ink-soft">
        {open ? 'Hide the full price' : label}
      </button>
      {open && <p className="body-copy mt-2 text-ink-soft">{children}</p>}
    </div>
  );
}
