...
export default function ExperiencesPage() {
  return (
    <>
      {/* Hero — copy column left on linen, image filling the right. */}
      <section className="grid items-stretch md:grid-cols-[1fr_1.4fr]">
        <div className="order-2 flex items-center px-6 py-16 md:order-1 md:px-12 md:py-24">
          <div className="max-w-sm">
            ...
            <p className="body-copy mb-10 text-ink-soft">{hero.body}</p>
            <Link
              href={hero.linkHref}
              className="eyebrow inline-block rounded-full bg-forest px-7 py-3.5 !tracking-[0.12em] text-linen transition-opacity hover:opacity-90"
            >
              {hero.linkLabel}
            </Link>
          </div>
      ...
      {/* The stages */}
      <Section id="stages" bg="bg-shell">
        <div className="reveal mb-16 text-center">
          <p className="eyebrow mb-4 text-olive">{stages.label}</p>
        ...
        <ol className="relative mx-auto max-w-2xl">
          ...
          <div className="absolute bottom-6 left-[38px] top-6 hidden w-px bg-ink/25 sm:block" aria-hidden="true" />
          ...
          {stages.items.map((stage) => (
            <li key={stage.n} className="reveal relative grid grid-cols-[auto_1fr] items-center gap-5 py-7 sm:grid-cols-[32px_36px_128px_1fr] sm:gap-7 md:py-9">
              <span className="display text-lg text-ink-soft sm:text-xl" aria-hidden="true">
                {stage.n}
              </span>

              {/* Dot on the rule */}
              <span className="relative hidden justify-self-center sm:block" aria-hidden="true">
                <span className="block h-[7px] w-[7px] rounded-full bg-ink/70" />
              </span>
              ...
              <div className="col-span-2 sm:col-span-1">
                ...
                <p className="body-copy mb-2 text-sm text-ink-soft">{stage.age}</p>
                <p className="body-copy max-w-xs text-sm italic text-ink-soft">{stage.body}</p>
              </div>
            </li>
          ))}
        </ol>
      ...
      <Section>
        <div className="reveal mb-14 text-center">
          <p className="eyebrow mb-4 text-olive">{regular.eyebrow}</p>
        ...
        <ul className="reveal grid gap-10 sm:grid-cols-2 lg:grid-cols-3">
          {regular.items.map((item) => (
            <li key={item.name} className="border-t border-ink/15 pt-6">
              <h3 className="display mb-3 text-xl">{item.name}</h3>
              <p className="body-copy text-ink-soft">{item.body}</p>
            </li>
          ))}
        </ul>
      </Section>
      ...
      <Section bg="bg-shell">
        <div className="reveal mb-14 text-center">
          <p className="eyebrow mb-4 text-olive">{personalised.eyebrow}</p>
      ...
      <VideoBand src={closing.video} poster={closing.poster} minHeight="min-h-[60vh]" overlay="bg-black/35">
        <div className="mx-auto w-full max-w-[1400px] px-6 py-24 text-center md:px-12">
          <p className="eyebrow mb-6 text-linen/90">{closing.eyebrow}</p>
          <h2 className="display mx-auto mb-4 max-w-3xl text-[clamp(1.75rem,1.2rem+2.4vw,3rem)] text-linen">{closing.heading}</h2>
          <p className="body-copy mx-auto max-w-xl text-linen/90">{closing.sub}</p>
        </div>
      </VideoBand>
    </>
  );
